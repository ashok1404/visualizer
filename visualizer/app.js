...
// of those never carry a stack trace at all — only eMetadata + msg.
...
function parseEMetadataString(raw) {
  if (!raw) return null;
  let s = raw.trim();
...
function parsePayload(raw) {
  ...
  // eMetadata may arrive as a real object, a JSON string, or a Swift-style
  // bracket dump ("[title: \"...\", signal: 11, ...]") — accept all three.
  if (nativeApp.eMetadata) {
    if (typeof nativeApp.eMetadata === 'object') {
      metadata = nativeApp.eMetadata;
    } else if (typeof nativeApp.eMetadata === 'string') {
      try {
        metadata = JSON.parse(nativeApp.eMetadata);
      } catch (e) {
        metadata = parseEMetadataString(nativeApp.eMetadata);
      }
    }
  }
